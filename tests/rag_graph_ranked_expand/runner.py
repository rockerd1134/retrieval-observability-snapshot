...
seed_top_k = max(1, int(config.get("seed_top_k", config.get("top_k", 3))))
neighbor_budget = max(0, int(config.get("neighbor_budget", 2)))
...
with (output_dir / "predictions.jsonl").open("w", encoding="utf-8", newline="\n") as handle:
    for question in questions:
        ...
        neighbor_ids = set().union(*(edges.get(doc_id, set()) for doc_id in seed_ids)) - set(seed_ids)
        neighbor_ids &= set(corpus)
        neighbors = bm25_scores(query_tokens, corpus_tokens, neighbor_ids)[:neighbor_budget]
        selected = [(score, doc_id, "seed", rank) for rank, (score, doc_id) in enumerate(seeds, start=1)]
        selected.extend((score, doc_id, "ranked_neighbor", rank) for rank, (score, doc_id) in enumerate(neighbors, start=1))
        context = [
            {
                "doc_id": doc_id,
                "text": answer_text(corpus[doc_id], max_chars=max_context_chars),
                "score": float(score),
                "source": "graph_ranked_expand",
                "selection_role": role,
                "rank": rank,
            }
            for score, doc_id, role, rank in selected
        ]
